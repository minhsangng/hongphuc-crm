...
export const users = pgTable("users", {
  ...
  dob: date("dob").notNull().defaultNow(),
  loginName: varchar("login_name").notNull().unique(),
  password: text("password").notNull(),
  phoneNumber: varchar("phone_number", { length: 12 }).notNull().default("N/A"),
...
export const childrens = pgTable("childrens", {
  ...
  parentName: varchar("parent_name", { length: 30 }).notNull().default("N/A"),
  guardianship: varchar("guardianship", { length: 10 }).notNull().default("Mẹ"),
  bankName: varchar("bank_name", { length: 50 }).notNull().default("N/A"),
  bankNumber: varchar("bank_number", { length: 20 }).notNull().default("N/A"),
  phoneNumber: varchar("phone_number", { length: 12 }).notNull().default("N/A"),
  subsidyType: varchar("subsidy_type", { length: 50 }).notNull().default("Bình thường"),
  status: varchar("status").notNull().default("Đang học"),
});

export const semesters = pgTable("semesters", {
  id: integer("id").primaryKey().generatedAlwaysAsIdentity(),
  semesterName: varchar("semester_name", { length: 30 }).notNull().default("HK1"),
  schoolYear: varchar("school_year", { length: 20 }).notNull(),
  startDate: date("start_date").notNull().defaultNow(),
  endDate: date("end_date").notNull().default(sql`CURRENT_DATE + INTERVAL '4 months'`),
...
export const fees = pgTable("fees", {
  ...
  childrenId: integer("children_id").notNull(),
  total: numeric("total", { precision: 10, scale: 0 }).notNull().default(1690000),
...
export const healthRecords = pgTable("health_records", {
  ...
  childrenId: integer("children_id").notNull(),
  teacherId: integer("teacher_id").notNull(),
  weight: numeric("weight", { precision: 3, scale: 1 }).notNull(),
  height: numeric("height", { precision: 3, scale: 0 }).notNull(),
